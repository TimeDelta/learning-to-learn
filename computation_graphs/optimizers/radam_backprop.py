...
class RAdamBackprop(nn.Module):
    ...
    def forward(
        ...
        for (name, param), grad in zip(named_parameters, grads):
            ...
            if rho_t > 4:
                # Compute rectification term r (scalar) and promote to tensor for broadcasting
                r_scalar = math.sqrt(((rho_t - 4) * (rho_t - 2) * rho_inf) / ((rho_inf - 4) * (rho_inf - 2) * rho_t))
                r = torch.tensor(r_scalar, dtype=param.dtype, device=param.device)
                # RAdam update
