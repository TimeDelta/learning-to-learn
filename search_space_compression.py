    ...
    def forward(self, node_types, edge_index, node_attributes, batch):
        ...
        if len(node_attributes) > 0:
            attr_embedding = torch.stack(
                [self.attr_encoder(attrs) for graph in node_attributes for attrs in graph], dim=0
            )
        else:
        ...
        for l in range(latent.size(0)):
            ...
            for embedding in node_embeddings:
                ...
                while True:
                    ...
                    raw_dim = self.attr_dims_head(embedding)
                    raw_dim = torch.nan_to_num(raw_dim, nan=0.0, posinf=10.0, neginf=-10.0)
                    attr_dim_val = F.softplus(raw_dim).item()
                    if math.isnan(attr_dim_val) or math.isinf(attr_dim_val):
                        attr_dim_val = 1.0
                    attr_dims = max(1, int(math.ceil(attr_dim_val)))
                    values = []
