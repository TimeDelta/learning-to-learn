...
class Task:
    ...
    def __init__(self, metrics:List[Metric], num_samples:int, train_ratio:float, inputs:np.ndarray, outputs:np.ndarray, silent=False):
...
class RegressionTask(Task):
    ...
    def __init__(self, true_dims:int, observed_dims:int, metrics:List, num_samples:int, train_ratio:float, silent=False):
...
TASK_FEATURE_DIMS = {n: 4*len(c.random_init(num_samples=4, silent=True).feature_functions) for n, c in TASK_TYPE_TO_CLASS.items()}
